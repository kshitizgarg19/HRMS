...
function SidebarNav({ me, onNavigate }: { me: SessionUser; onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto px-3 py-4">
      {NAV.map((group) => {
        ...
        return (
          <div key={group.title}>
            <p className="mb-1.5 px-3 text-[10px] font-extrabold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">{group.title}</p>
...
function NotificationBell() {
  ...
  useEffect(() => {
    api<{ items: typeof items }>("/api/notifications").then((d) => setItems(d.items)).catch(() => {});
  }, []);
      ...
      <ToastProvider>
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
          ...
          {mobileOpen && (
            <div className="fixed inset-0 z-40 lg:hidden">
              ...
              <aside className="absolute inset-y-0 left-0 flex w-72 flex-col bg-white shadow-2xl dark:bg-slate-900">
                ...
                <SidebarNav me={me} onNavigate={() => setMobileOpen(false)} />
              </aside>
            </div>
          )}
          ...
          <div className="lg:pl-64">
            <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur-md sm:px-6 dark:border-slate-800/80 dark:bg-slate-900/80">
              <button onClick={() => setMobileOpen(true)} className="grid size-10 place-items-center rounded-xl text-slate-500 hover:bg-slate-100 lg:hidden cursor-pointer dark:text-slate-400 dark:hover:bg-slate-800">
                <Menu size={20} />
              </button>
              <form
            ...
            <main key={pathname} className="page-enter mx-auto max-w-[1400px] p-4 sm:p-6">{children}</main>
          </div>
        </div>
      </ToastProvider>
    </MeCtx.Provider>
